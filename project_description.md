# 1. TRANSCRIÇÃO COMPLETA DO PDF DO PROJETO

MC504 - Sistemas Operacionais
Projeto de Animação Multithread
Projeto em grupo de no máximo 4 estudantes
Peso na média de projetos: 2

Objetivo: implementação de uma aplicação multithread que utiliza semáforos e/ou mutex locks e variáveis de condição para sincronização e de um visualizador para o progresso desta aplicação.

Os problemas descritos no livro *The Little Book of Semaphores* de Allen B. Downey são uma boa fonte de inspiração. O problema dos filósofos (analisado em sala de aula) e o do barbeiro dorminhoco, descrito abaixo, não deverão ser escolhidos. Você e/ou seu grupo também podem propor ou escolher problemas de complexidade semelhante aos do livro.

## Exemplo: O Barbeiro Dorminhoco
Neste problema clássico, há um barbeiro, uma cadeira de atendimento e um número limitado de cadeiras na sala de espera. Quando não há clientes para serem atendidos, o barbeiro dorme.

Figura 2.35: Tanenbaum (Ilustração de um barbeiro dormindo na cadeira, 5 cadeiras vazias na sala de espera, um cliente com muito cabelo entrando e um cliente careca saindo).

Quando um cliente chega, ele deve esperar em uma cadeira da sala de espera até poder ser atendido. Se não houver cadeiras disponíveis, o cliente deve ir embora.

Um método simples para o acompanhamento passo a passo do algoritmo seria gerar um log, em que cada ação feita por um dos atores é reportada por uma string.

### Log de exemplo:
Cliente 1 entrou na barbearia.
Cliente 2 entrou na barbearia.
Cliente 1 sentou na cadeira do barbeiro.
Barbeiro iniciou o corte de cabelo do Cliente 1.

Este tipo de saída, no entanto, não permite de maneira simples a visualização do estado global da aplicação. Observe agora esta proposta que utilizando apenas caracteres ASCII ilustra o estado completo de uma barbearia com dois barbeiros e cinco cadeiras na sala de espera.

BARBEARIA
... /                                  zzz   zzz \
  2 |                                            |
    \   C00   C01   C02   C03   C04    B00   B01 /
...
O código desta aplicação está disponível no material da aula de 07 de maio (abaixo).

## Entregas:
1. Um link para um repositório com o código implementado. Lembre-se de fazer um arquivo README com informações relevantes e de respeitar boas práticas de programação e de documentação. O uso da biblioteca pthreads é recomendado para o desenvolvimento da aplicação, mas o visualizador pode ser codificado em outra linguagem de programação. O uso de bibliotecas gráficas, como ncurses, é opcional.
2. Um vídeo com uma apresentação do projeto, com descrição do problema abordado (por exemplo, Festa na República) e de características da animação desenvolvida (quais são os elementos, qual foi a abordagem), além de trechos do programa sendo executado. Caso tenha utilizado apoio de ferramentas do tipo LLM para a implementação do projeto, inclua as interações mais relevantes, sua análise crítica sobre os resultados obtidos e qual foi sua contribuição para a realização do projeto a partir destes resultados.

## Pontos que serão levados em consideração para a avaliação:
* **Tema escolhido e visualização:**
    * O problema escolhido permite uma boa exploração dos conceitos de sincronização?
    * A animação permite uma compreensão do problema abordado e da evolução da execução ao longo do tempo?
    * A implementação permite variação de parâmetros?
    * A abordagem proposta foi criativa?
* **Código:**
    * O repositório está bem organizado?
    * O arquivo README contém informações relevantes?
    * O código está claro e bem comentado?
    * As boas práticas de programação foram respeitadas?
* **Vídeo:**
    * O problema abordado foi descrito claramente?
    * As principais decisões do grupo e características da implementação foram apresentadas?
    * Foi apresentada pelo menos uma execução da aplicação rodando?
    * Há coerência entre o tempo do vídeo e o conteúdo apresentado?
    * O vídeo apresenta elementos que tornam a experiência de quem assiste mais agradável como, por exemplo, slides bem feitos, imagens interessantes, bom ritmo de apresentação ou bom humor?
